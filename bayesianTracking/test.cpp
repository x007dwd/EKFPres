...
public:
  ...
  void doProcess(double DeltaTime, double observationRange,
...
protected:
   ...
   * being the number of "observations": how many observed landmarks for a map,
   * or just one if not applicable.
    * \param out_data_association An empty vector or, where applicable, a vector
   * where the i'th element corresponds to the position of the observation in
   * the i'th row of out_z within the system state vector (in the range
   ...
   * landmarks in the map. Note that, in contrast, in_S only comprises a subset
   * of all the landmarks.
    * \param in_S The full covariance matrix of the observation predictions
   * (i.e. the "innovation covariance matrix"). This is a M·O x M·O matrix with
   * M=length of "in_lm_indices_in_S".
   ...
   * [0,getNumberOfLandmarksInTheMap()-1]) that can be found in the matrix in_S.
    ...
    * \note It is assumed that the observations are independent, i.e. there are
   * NO cross-covariances between them.
    */
   ...
   * whose predictions are expected as output. For non SLAM-like problems, this
   * input value is undefined and the application should just generate one
   * observation for the given problem.
    * \param out_predictions The predicted observations.
    */
  void OnObservationModel(const vector_size_t &idx_landmarks_to_predict,
   ...
   * zero and the expected output is for the whole state vector.
    ...
    */
  void OnObservationJacobians(const size_t &idx_landmark_to_predict,
                              KFMatrix_OxV &Hx, KFMatrix_OxF &Hy) const;
   ...
   * topology of the individual scalar components (eg, angles).
    */
  void OnSubstractObservationVectors(KFArray_OBS &A,
                                     const KFArray_OBS &B) const;
    ...
    : public mrpt::bayes::CParticleFilterData<CParticleVehicleData>,
      public mrpt::bayes::CParticleFilterDataImpl<
          ...
          mrpt::bayes::CParticleFilterData<
              CParticleVehicleData>::CParticleList> {
public:
...
void TestBayesianTracking() {
  ...
  while (winEKF.isOpen() && winPF.isOpen() && !mrpt::system::os::kbhit()) {
    ...
    float obsBearing =
        realBearing +
        BEARING_SENSOR_NOISE_STD * randomGenerator.drawGaussian1D_normalized();
    ...
    // Process with PF:
    CSensoryFrame SF;
    CObservationBearingRangePtr obsRangeBear =
...
void CRangeBearing::doProcess(double DeltaTime, double observationRange,
                              double observationBearing) {
...
void CRangeBearingParticleFilter::prediction_and_update_pfStandardProposal(
  ...
  for (i = 0; i < N; i++) {
    m_particles[i].d->x +=
        ...
        TRANSITION_MODEL_STD_XY * randomGenerator.drawGaussian1D_normalized();
    m_particles[i].d->y +=
        DELTA_TIME * m_particles[i].d->vy +
        TRANSITION_MODEL_STD_XY * randomGenerator.drawGaussian1D_normalized();

    m_particles[i].d->vx +=
        TRANSITION_MODEL_STD_VXY * randomGenerator.drawGaussian1D_normalized();
    m_particles[i].d->vy +=
        TRANSITION_MODEL_STD_VXY * randomGenerator.drawGaussian1D_normalized();
  }
  ...
  for (i = 0; i < N; i++) {
    ...
    m_particles[i].log_w +=
        log(math::normalPDF(predicted_range - obsRange, 0,
                            RANGE_SENSOR_NOISE_STD)) +
        log(math::normalPDF(math::wrapToPi(predicted_bearing - obsBearing), 0,
...
void CRangeBearingParticleFilter::initializeParticles(size_t M) {
  ...
  for (CParticleList::iterator it = m_particles.begin();
       it != m_particles.end(); it++) {
    (*it).d->x = randomGenerator.drawUniform(VEHICLE_INITIAL_X - 2.0f,
                                             VEHICLE_INITIAL_X + 2.0f);
    (*it).d->y = randomGenerator.drawUniform(VEHICLE_INITIAL_Y - 2.0f,
                                             VEHICLE_INITIAL_Y + 2.0f);
...
/** Computes the average velocity
  */
void CRangeBearingParticleFilter::getMean(float &x, float &y, float &vx,
                                          float &vy) {
  double sumW = 0;
  for (CParticleList::iterator it = m_particles.begin();
       it != m_particles.end(); it++)
